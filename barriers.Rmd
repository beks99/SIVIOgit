...
barriers_critera = data %>%
  group_by(across(all_of(level))) %>%
  summarise(n=n()) %>%
  full_join(y = (barrier_reasons %>% select(all_of(level), knowledge, documentation, legal, finance, inflation, law, trust, bureaucracy, inappropriate)), 
            by = all_of(level)) %>%
