...
index = data %>%
  ...
  summarise(n=n()) %>%
  full_join(y = (barriers_data %>% select(all_of(level), barriers_score, barriers_normalise)), by = all_of(level)) %>%
  full_join(y = (compliance_data %>% select(all_of(level), compliance_score, compliance_normalise)), by = all_of(level)) %>%
  full_join(y = (availability_data %>% select(all_of(level), availability_score, availability_normalise)), by = all_of(level)) %>%
  full_join(y = (usage_data %>% select(all_of(level), usage_score, usage_normalise)), by = all_of(level)) %>%
...
index$index_score = rowSums(index[, c("barriers_inclusion", "compliance_inclusion", "availability_inclusion", "usage_inclusion")], na.rm = T)
