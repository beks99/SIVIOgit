...
MSMEs who use their bank account once oer month
### frequency

- frequency_bank_account == "Daily" ~ 1,

- frequency_bank_account == "Several times per week" ~ 1,

- frequency_bank_account == "Once a week" ~ 1,

- frequency_bank_account == "Once per month" ~ 1,

- frequency_bank_account == "Once per 3 months" ~ 0,

- frequency_bank_account == "Once per 6 months" ~ 0,

- frequency_bank_account == "Once per annum" ~ 0,
...
frequency_bank_account = data %>%
  ...
  group_by(across(all_of(level))) %>%
  summarise(frequency_bank_account = sum(frequency, na.rm = T),
            responses = n(),
            not_answered_frequency_bank = sum(not_answered)) %>%
  mutate(frequency_bank_account_value = frequency_bank_account / (responses - not_answered_frequency_bank))
...
MSMEs accessing 3 or more of the services
Scoring

- number_services >= 5 ~ 1,

- number_services: 3-4 ~ 1,
...
```{r echo=FALSE}
 number_bank_services = data %>%
  ...
  group_by(across(all_of(level))) %>%
  summarise(number_bank_services = sum(number_services_score, na.rm = T),
            responses = n(),
            not_answered_bank_services = sum(not_answered)) %>%
  mutate(number_bank_services_value = number_bank_services / (responses - not_answered_bank_services))
...
MSMEs accessing 5 or more of the services about
Scoring

- number_services >= 5 ~ 1,

- number_services: 2-4 ~ 1,
...
MSMEs who use mobile money account at least once a week

- frequency_mobile_money == "Everyday" ~ 1,

- frequency_mobile_money == "Two to 3 times per week" ~ 1,

- frequency_mobile_money == "Once a week" ~ 1,

- frequency_mobile_money == "Once per month" ~ 0,

- frequency_mobile_money == "Once every 3 months" ~ 0,

- frequency_mobile_money == "Once every 6 months" ~ 0,

- frequency_mobile_money == "Once per annum" ~ 0,
...
frequency_mobile_money = data %>%
  ...
  group_by(across(all_of(level))) %>%
  summarise(frequency_mobile_money = sum(frequency, na.rm = T),
            responses = n(),
            not_answered_frequency_bank = sum(not_answered)) %>%
  mutate(frequency_mobile_money_value = frequency_mobile_money / (responses - not_answered_frequency_bank))
...
```{r}
write.csv(usage_critera, file = "~/SIVIOgit/FI_index/usage_criteria.csv")
```
