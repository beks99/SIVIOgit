...
success_loan = success_loan_12months %>%
  select(all_of(level), success_12months) %>%
  full_join((success_loan_4years %>% select(province, success_4years)), by = all_of(level))

success_loan %>%
  mutate(success_loan_value = rowMeans(success_loan[, c("success_12months", "success_4years")])) %>%
  kable() %>%
  kable_styling()
```
...
service_loan = service_loan_12months %>%
  select(all_of(level), service_12months) %>%
  full_join((service_loan_4years %>% select(all_of(level), service_4years)), by = all_of(level))

service_loan %>%
  mutate(service_loan_value = rowMeans(service_loan[, c("service_12months", "service_4years")])) %>%
  kable() %>%
  kable_styling()
```
...
interest_commercial = interest_12months %>%
  select(all_of(level), interest_rate_commercial_loan_12months) %>%
  full_join((interest_4years %>% select(all_of(level), interest_rate_commercial_loan_4years)), by = all_of(level))

interest_commercial %>%
  mutate(interest_commercial_value = rowMeans(interest_commercial[, c("interest_rate_commercial_loan_12months", "interest_rate_commercial_loan_4years")])) %>%
  kable() %>%
  kable_styling()
```
...
payback_commercial = payback_12months %>%
  select(all_of(level), payback_commercial_loan_12months_months) %>%
  full_join((payback_4years %>% select(all_of(level), payback_commercial_loan_4years_months)), by = all_of(level))

payback_commercial %>%
  mutate(payback_commercial_value = rowMeans(payback_commercial[, c("payback_commercial_loan_12months_months", "payback_commercial_loan_4years_months")], na.rm =T)) %>%
  kable() %>%
  kable_styling()
```
...
data %>%
  ...
  group_by(across(all_of(level))) %>%
  summarise(number_services = mean(number_services_score),
            responses = n(),
            not_answered_services_other = sum(not_answered)) %>%
  mutate(number_services_value = number_services / 3)
```
...
data %>% select(all_of(level),
  ...
  group_by(across(all_of(level))) %>%
  summarise(formal_loan = sum(formal_loan),
            informal_loan = sum(informal_loan),
            responses = n(),
            not_answered_loan_other = sum(not_answered)) %>%
  mutate(formal_loan_value = formal_loan / (responses - not_answered_loan_other))
```
...
insurance_5years = insurance_12months %>%
  select(all_of(level), comprehesive_12months, product_12months, staff_12months) %>%
  full_join((insurance_4years %>% select(all_of(level), comprehesive_4years, product_4years, staff_4years)), by = all_of(level))

insurance_5years %>%
  mutate(comprehensive_value = rowMeans(insurance_5years[, c("comprehesive_12months", "comprehesive_4years")]),
         product_value = rowMeans(insurance_5years[, c("product_12months", "product_4years")]),
         staff_value = rowMeans(insurance_5years[, c("staff_12months", "staff_4years")])) %>%
  kable() %>%
  kable_styling()
```
